...
async def run_ws(uri: str) -> None:
    ...
    async def connect_and_play(target_uri: str, headers: Optional[Dict[str, str]]) -> None:
        nonlocal token
        bot_states: Dict[int, bot.BotState] = {}
            ...
            try:
                async with websockets.connect(target_uri, **kwargs) as ws:
                    ...
                    async for raw in ws:
                        ...
                        sim = json.loads(json.dumps(data))
                        bots = sim.get("bots", [])
                        bot_ids = sorted(int(b.get("id", i)) for i, b in enumerate(bots))
                        bot_id_to_index = {int(b.get("id", i)): i for i, b in enumerate(bots)}

                        actions: List[Dict[str, Any]] = []
                        reserved_item_ids = set()
                        for bid in bot_ids:
                            st = bot_states.setdefault(bid, bot.BotState())
                            act = bot.choose_action(sim, st, bid, reserved_item_ids=reserved_item_ids)
                            actions.append(act)
                            if act.get("action") == "pick_up" and act.get("item_id"):
                                reserved_item_ids.add(str(act["item_id"]))
                            elif st.target_item_id:
                                reserved_item_ids.add(str(st.target_item_id))
                            bot.apply_simulated_action(sim, act, bot_id_to_index)

                        await ws.send(json.dumps({"actions": actions}))
                return
            except Exception as exc:
                with lock:
                    status_line = f"Retry: {type(exc).__name__}"
                continue
...
def main() -> None:
    ...
    if not uri:
        raise SystemExit("Sett GROCERY_WS med wss://...token=...")

    t = threading.Thread(target=lambda: asyncio.run(run_ws(uri)), daemon=True)
    t.start()
    run_visual()
